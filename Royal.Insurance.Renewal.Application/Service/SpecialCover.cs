...
namespace Royal.Insurance.Renewal.Application.Service
{
    public class SpecialCover : CommonImplimentation ,IPremiumCalculation
    {
        ...
        public override OutPutDTO GetPremiumResult(InputDTO inputDtos)
        {
            ...
            if (inputDtos.PayOutAmount > 2000000)
            {
                if (getDiscount > 0)
                {
        ...
        {
            if (inputDto.PayOutAmount >= 2000000)
            {
                throw new System.Exception("Premium value should not exceed 2000000 " + inputDto.CustomerId);
            }
